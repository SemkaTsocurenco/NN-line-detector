...
def create_two_lane_mask(shape=(384, 384)):
    ...
    # Left line (goes through left half)
    left_poly = [0.0003, -0.15, 150]
    x_vals_left = np.arange(30, w // 2 + 30, 2)
    y_vals_left = np.polyval(left_poly, x_vals_left)
    y_vals_left += np.random.randn(len(y_vals_left)) * 2  # Add noise
    ...
    # Right line (goes through right half)
    right_poly = [0.0004, -0.2, 200]
    x_vals_right = np.arange(w // 2 - 30, w - 30, 2)
    y_vals_right = np.polyval(right_poly, x_vals_right)
    y_vals_right += np.random.randn(len(y_vals_right)) * 2  # Add noise
...
def test_line_splitting():
    ...
    for line in lines_no_split:
        ...
        if x_start != x_end:
            ...
            y_values = np.polyval(line.poly_coeffs, x_values)
    ...
    for i, line in enumerate(lines_with_split):
        ...
        if x_start != x_end:
            ...
            mid_y = int(np.polyval(line.poly_coeffs, mid_x))
